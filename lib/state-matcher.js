...
class WindowStateInfo {
    constructor(winid, state = WindowState.PENDING) {
        ...
        // A user grab during startup takes ownership of placement.  Restore
        // operations must not win a race against an active move/resize.
        this.userGrabActive = false
        this.userOverrodePlacement = false
...
const DISCORD_CLASSES = new Set(['vesktop', 'discord', 'discordcanary', 'discordptb'])

function isDiscordMainWindow(details) {
    const wmClass = (details.wm_class || '').toLowerCase()
    if (!DISCORD_CLASSES.has(wmClass)) return false
    // Unread counts and notification dots can both prefix the title.
    const title = (details.title || '').trim().replace(/^(?:(?:[•·]\s*)|(?:\(\d+\)\s*))+/u, '')
    return /^(?:Discord|Vesktop)(?:\s*\|.*)?$/i.test(title) ||
        // Vesktop displays its offline page in the same resizable main
        // window. This is not a new document or a separate error dialog.
        (wmClass === 'vesktop' && title === 'Network Error')
}
...
class WindowStateMatcher {
    ...
    _onSettleTimeout(winid, state) {
        if (state.userGrabActive || state.userOverrodePlacement ||
            this._windowStates.get(winid) !== state || state.state !== WindowState.SETTLING) return
    ...
    restoreFromState(windowsState) {
        ...
        // carry occupancy of currently-live windows across the reload, keyed by
        // identity. a runtime saved-windows reapply (e.g. an external prefs
        // write) must not strand a settled window: without this every slot comes
        // back unoccupied while its window-state machine stays TRACKING, so the
        // window's next title change is misread as a brand-new window and
        // re-matched onto the wrong slot, moving it. at enable time knownWindows
        // is empty, so this is a no-op and the constructor's actor query matches
        const liveOccupancy = new Map() // identityKey -> queue of live winids
        for (const w of this.knownWindows) {
            if (w.occupied != null) {
                const key = this._slotIdentityKey(w.props)
                if (!liveOccupancy.has(key)) liveOccupancy.set(key, [])
                liveOccupancy.get(key).push(w.occupied)
            }
        }

        this.knownWindows = windowsState.map((w) => {
            const props = this.deepCopyProps(w.props)
            let occupied = this.preserveOccupiedState ? (w.occupied || null) : null
            if (occupied == null) {
                const queue = liveOccupancy.get(this._slotIdentityKey(props))
                if (queue && queue.length) occupied = queue.shift()
            }
            return { occupied, props, seen: w.seen || 0 } // seen 0 (epoch) for old entries
        })

        // Don't save immediately after restoration - state was just loaded
    }

    _slotIdentityKey(props) {
        return `${props?.wm_class || ''}\n${props?.title || ''}`
    }
    ...
    updateOrAddConfig(props, newStateProps, updateIdentity = true) {
        ...
        // Update identity properties at top level unless this is a startup
        // placement explicitly chosen by the user.  In that case preserve the
        // saved identity until the real title arrives.
        if (updateIdentity) {
    ...
    getPolicyForWindow(details) {
        ...
        const defaults = {
            ...
            // Discord's main window title names the current conversation,
            // not a separate document. Popouts/updaters retain title matching.
            matchByClass: isDiscordMainWindow(details),
        }
        ...
        return {
            ...
            matchByClass: matchedRule.match_by_class ?? defaults.matchByClass,
        }
    }
    ...
    calculateScoresForWindow(pendingDetails) {
        const pendingTitle = pendingDetails?.title || ''
        const separateDiscordRoles = DISCORD_CLASSES.has((pendingDetails.wm_class || '').toLowerCase())
        const mainWindow = this.getPolicyForWindow(pendingDetails).matchByClass
        // A same-title window from another application must never outrank
        // the right application's candidate (even with a lower score).
        const unoccupiedKnownWindows = this.knownWindows.filter((w) =>
            !w.occupied && w.props.wm_class === pendingDetails.wm_class &&
            (!separateDiscordRoles || this.getPolicyForWindow(w.props).matchByClass === mainWindow))
        ...
        const scores = unoccupiedKnownWindows.map((w) => ({
            ...
            positionMatch: this._slotMatchesCurrentState(pendingDetails, w),
        }))

        // Sort by exact title (desc), score (desc), then position match (desc)
        // Position tiebreaker prevents identical-title windows from swapping
        // slots on extension re-enable when actor enumeration order differs
        scores.sort((a, b) =>
            (b.exactTitle - a.exactTitle) ||
            (b.score - a.score) ||
            (b.positionMatch - a.positionMatch) ||
            ((b.window.seen || 0) - (a.window.seen || 0)))
        return scores
    }

    _classFallback(details, winid) {
        const policy = this.getPolicyForWindow(details)
        const candidates = this.knownWindows.filter(w => !w.occupied &&
            w.props.wm_class === details.wm_class && this.getBestAvailableConfig(w.props))
        if (policy.matchByClass) {
            // An already-open main window owns its slot; don't hand an old
            // conversation's slot to a new popout or a simultaneous instance.
            if ([...this._windowStates.values()].some(s =>
                s.winid !== winid && s.details?.wm_class === details.wm_class &&
                this.getPolicyForWindow(s.details).matchByClass)) return null
            const mainCandidates = candidates.filter(w => this.getPolicyForWindow(w.props).matchByClass)
            return mainCandidates.sort((a, b) => (b.seen || 0) - (a.seen || 0))[0] || null
        }
        return null
    }

    // Check if a window's current state matches a slot's saved config.
    // Returns a numeric score: 2 = workspace+connector match, 1 = workspace only, 0 = no match
    _slotMatchesCurrentState(details, knownWindow) {
        const configs = knownWindow.props?.configs
        if (!configs?.length) return 0
        const ws = details.workspace
        if (ws === undefined) return 0
        const connector = details.monitor !== undefined
            ? this.getConnectorForMonitorCallback?.(details.monitor) : null
        let best = 0
        for (const cfg of configs) {
            if (cfg.workspace === ws) {
                const connMatch = connector && cfg.connector === connector
                best = Math.max(best, connMatch ? 2 : 1)
            }
        }
        return best
    }
    ...
    // empty titles carry no identity, so they never count as exact
    hasExactMatch(pendingDetails) {
        const pendingTitle = pendingDetails?.title || ''
        if (!pendingTitle) return false

        return this.knownWindows.some(
            (w) =>
                !w.occupied &&
                w.props.wm_class === pendingDetails.wm_class &&
                (w.props?.title || '') === pendingTitle
        )
    }
    ...
    shouldDecideOnWindow(windowState, currentTime, isTimedOut) {
        ...
        const hasExactMatch = this.hasExactMatch(details) || Boolean(this._classFallback(details, windowState.winid))
        if (hasExactMatch) {
            return {
                decide: true,
                debug: {
        ...
        // generic titles carry weak identity: only decide early when there is
        // no same-class slot to mismatch against, otherwise wait for the title
        // to become specific or the extended timeout to expire. deciding early
        // on a generic title is what previously forced corrective moves after
        // the window had already settled.
        const hasCandidateSlots = this.knownWindows.some(
            (w) => !w.occupied && w.props.wm_class === details.wm_class)
        const decision = titleIsStable
            ? isIdle || (hasBeenIdleLongEnough && isTimedOut)
            : isTimedOut || (isIdle && !hasCandidateSlots)
    ...
    generateRestoreOperations(winid, currentDetails, knownProps, policy = null, force = false, directConfig = null) {
        ...
        const generation = this._getWindowState(winid).operationGeneration
        const allowed = this.operationFilter ? operations.filter(this.operationFilter) : operations
        return this.expandOperations(allowed, currentDetails).map(op => ({ ...op, generation }))
    }
    ...
    matchWindowToKnown(winid, details, bestMatch, policy = null) {
        ...
        const userOwnsPlacement = windowState.userGrabActive || windowState.userOverrodePlacement

        // Generate operations for window state restoration
        const operations = userOwnsPlacement
            ? []
            : this.generateRestoreOperations(winid, details, bestMatch.props, policy)
        ...
        if (operations.length > 0) {
        ...
        } else if (targetConfig || userOwnsPlacement) {
            if (userOwnsPlacement) {
                // The user deliberately chose the startup placement.  Save the
                // geometry but do not replace the remembered identity with a
                // transient loading title.
                this.updateOrAddConfig(bestMatch.props, details, false)
            }
            ...
            if (userOwnsPlacement && !windowState.userGrabActive) windowState.userOverrodePlacement = false
        }
    ...
    addWindowAsNew(winid, details) {
        ...
        // A presentation deadline may have suppressed matching before this
        // genuinely new window was learned. Its new slot now owns the actual
        // placement; normal future monitor changes must work again.
        const state = this._getWindowState(winid)
        if (!state.userGrabActive) state.userOverrodePlacement = false
    ...
    /**
     * Handle compositor user-grab lifecycle events.  A grab is authoritative
     * while a window is pending or being restored: delayed title matching and
     * queued restore operations must not snap it back afterward.
     */
    _handleUserGrabEvent(winid, eventName, coreDetails) {
        const state = this._getWindowState(winid)
        state.lastEventTime = Date.now()

        if (eventName === 'user-grab-begin') {
            state.operationGeneration++
            state.userGrabActive = true
            this._cancelSettleTimer(state)
            if (state.state !== WindowState.TRACKING) {
                state.userOverrodePlacement = true
                debug('state', `window ${winid} user grab began during ${state.state}; startup restore is now suppressed`)
            }
            // Drop the target immediately, not after the pointer is released.
            // Keeping a settle timer alive can otherwise restart a restore mid-drag.
            state.targetConfig = null
            if (state.state !== WindowState.PENDING) {
                this._transitionState(winid, WindowState.TRACKING, coreDetails, 'user took control')
            }
            state.details = coreDetails
            return {
                operations: [],
                events: [{ type: 'window.user_grab_begin', winid, details: coreDetails }],
            }
        }

        state.userGrabActive = false

        // Save the final geometry even if no debounced position event follows.
        // A drag across monitors chooses a new position, not the old position
        // remembered on that monitor.
        const slot = this.findOccupiedSlots(winid)[0]
        if (slot && isValidGeometry(coreDetails.frame_rect)) {
            const connector = this.getConnectorForMonitorCallback?.(coreDetails.monitor)
            this.updateOrAddConfig(slot.props, coreDetails, !this.isLikelyGenericTitle(coreDetails.title))
            this._updateConnectorPreference(slot.props, connector)
            slot.seen = Date.now()
            this._transitionState(winid, WindowState.TRACKING, coreDetails, 'user geometry accepted')
            state.userOverrodePlacement = false
            this.notifyStateChange()
        }
        state.details = coreDetails

        let pendingResult = { operations: [], events: [] }
        if (state.state === WindowState.PENDING) {
            pendingResult = this.processPendingWindows()
        }

        return {
            operations: pendingResult.operations,
            events: [
                { type: 'window.user_grab_end', winid, details: coreDetails },
    ...
    /**
     * OperationHandler uses this to drop queued restore operations after the
     * user takes control of a window during startup.
     */
    shouldSkipOperation(winid, generation = undefined) {
        const state = this._windowStates.get(winid)
        return !state || Boolean(state.userGrabActive || state.userOverrodePlacement ||
            (generation !== undefined && generation !== state.operationGeneration))
    }

    getStartupStatus(winid, details) {
        if (!shouldTrackWindow(details, true) || this.getPolicyForWindow(details).action === 'IGNORE') {
            return { eligible: false, ready: true }
        }
        const state = this._windowStates.get(winid)
        const eligible = this.knownWindows.some(w =>
            (w.occupied === winid || (!w.occupied && w.props.wm_class === details.wm_class)) &&
            this.getBestAvailableConfig(w.props))
        const target = state?.targetConfig
        let ready = state?.state === WindowState.TRACKING
        if (state?.state === WindowState.SETTLING && target && details.frame_rect) {
            const props = this.findOccupiedSlots(winid)[0]?.props || details
            ready = this.generateRestoreOperations(winid, details, props,
                this.getPolicyForWindow(props), false, target).length === 0
        }
        return { eligible, ready, target }
    }

    _finishStartup(winid, details) {
        // Once the window is released for display, its startup placement is
        // final. Cancel both queued operations and drift correction; a late
        // title or an application-enforced size must not cause a visible jump.
        let state = this._windowStates.get(winid)
        if (!state && !shouldTrackWindow(details)) return { operations: [], events: [] }
        state ||= this._getWindowState(winid)
        state.operationGeneration++
        this._cancelSettleTimer(state)
        state.targetConfig = null
        state.details = details
        const slot = this.findOccupiedSlots(winid)[0]
        if (slot && isValidGeometry(details.frame_rect)) {
            this.updateOrAddConfig(slot.props, details, !this.isLikelyGenericTitle(details.title))
            this._transitionState(winid, WindowState.TRACKING, details, 'startup presentation released')
            state.userOverrodePlacement = false
            this.notifyStateChange()
        } else {
            state.userOverrodePlacement = true
        }
        return { operations: [], events: [{ type: 'window.startup_finished', winid, details }] }
    }

    processSinglePendingWindow(winid, windowState, currentTime, ambiguousWmClasses) {
        const details = windowState.details
        if (!details || windowState.userGrabActive) {
        ...
        const classFallback = this._classFallback(details, winid)
        const bestScore = classFallback ? 1 : (scores.length > 0 ? scores[0].score : -1)
        const secondBestScore = scores.length > 1 ? scores[1].score : -1
        const bestMatch = classFallback || (scores.length > 0 ? scores[0].window : null)
    ...
    _handleUserMonitorChange(winid, windowState, coreDetails, currentSlot, modifiedEvent) {
        if (windowState.userGrabActive || windowState.userOverrodePlacement) return null
    ...
    _handleOccupiedWindowModified(winid, coreDetails, currentSlot, modifiedEvent) {
        ...
        // a window that occupies a slot is never re-matched or moved because
        // its title changed: identity resolution happens exclusively while
        // PENDING. title changes here only update the stored identity below.
    ...
    /**
     * re-bind a window we still track but that lost its slot occupancy (e.g. a
     * saved-windows reapply blanked occupancy without re-matching). a settled
     * window must never be re-identified by its current title, so bind it back
     * to the slot matching its prior identity and let the normal occupied path
     * handle this event. if that slot is gone, re-learn it as new (no move).
     */
    _rebindKnownWindow(winid, windowState, coreDetails, modifiedEvent) {
        const prior = windowState.details || coreDetails
        const slot = this._findUnoccupiedSlotByIdentity(prior)
        if (slot) {
            slot.occupied = winid
            debug('state', `re-bound settled window ${winid} to its slot (occupancy was lost)`)
            return this._handleOccupiedWindowModified(winid, coreDetails, slot, modifiedEvent)
        }
        debug('state', `settled window ${winid} lost its slot with no identity match; re-learning as new`)
        const result = this.addWindowAsNew(winid, coreDetails)
        return { operations: result.operations, events: result.event ? [modifiedEvent, result.event] : [modifiedEvent] }
    }

    _findUnoccupiedSlotByIdentity(details) {
        const wmClass = details?.wm_class
        const title = details?.title || ''
        return this.knownWindows.find(
            (w) => !w.occupied && w.props?.wm_class === wmClass && (w.props?.title || '') === title
        ) || null
    }
    ...
    onWindowModified(winid, eventName, windowDetails) {
        ...
        if (eventName === 'startup-finished') {
            const existing = this._windowStates.has(winid)
            if (!existing && !shouldTrackWindow(details)) return { operations: [], events: [] }
            this._getWindowState(winid)
            return this._finishStartup(winid, coreDetails)
        }

        // User move/resize lifecycle must be handled before normal matching.
        // This lets a drag that begins while the title is still loading claim
        // the window before a delayed restore decision can be made.
        if (eventName === 'user-grab-begin' || eventName === 'user-grab-end') {
            return this._handleUserGrabEvent(winid, eventName, coreDetails)
        }
        ...
        // invariant: a window we already track is never demoted to PENDING and
        // re-identified at runtime. a genuinely new winid starts out PENDING, so
        // reaching here in any other state means it lost its slot occupancy
        // (e.g. a saved-windows reapply). re-bind it to its slot by its prior
        // identity and keep it settled, rather than re-matching on its current
        // (possibly transient) title and moving it to another window's slot
        if (windowState.state !== WindowState.PENDING) {
            return this._rebindKnownWindow(winid, windowState, coreDetails, modifiedEvent)
        }
