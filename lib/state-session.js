...
export class OperationHandler {
    ...
    processTrackerResult(result) {
        ...
        if (result.events && result.events.length > 0) {
            for (const event of result.events) {
                ...
                if (event.type === 'window.user_grab_begin' || event.type === 'window.destroyed' ||
                    event.type === 'window.startup_finished') {
                    this._cancelPendingOperations(event.winid)
                }
    ...
    _cancelPendingOperations(winid) {
        const pending = this._pendingMoves.get(winid)
        if (!pending) return
        if (pending.timeoutId) this._timeoutManager.remove(pending.timeoutId)
    ...
    _checkMoveCompletion(event) {
        ...
        if (pending.targetWorkspace !== undefined && details.workspace === pending.targetWorkspace &&
            !pending.settleScheduled) {
            // Subsequent loading/title/geometry events must not postpone this
            // batch indefinitely once the destination workspace is reached.
            pending.settleScheduled = true
    ...
    _executePendingOperations(winid) {
        ...
        const pending = this._pendingMoves.get(winid)
        // Synchronous compositor signals must not re-arm or replay this batch.
        this._pendingMoves.delete(winid)
        ...
        const userTookControl = this._tracker?.shouldSkipOperation?.(winid, pending.operations[0]?.generation)
        ...
        // reliably on non-visible workspaces.  If the user took control, skip
        // the workspace activation too: it is part of the restore we are
        // abandoning, not an independent action.
        if (!userTookControl && pending.targetWorkspace !== undefined &&
            this._executor?._config?.activate_on_move && this._executor?.activateWorkspace) {
            this._executor.activateWorkspace(pending.targetWorkspace)
        }

        // Execute queued operations unless the user took control while the
        // workspace transition was settling.
        for (const op of pending.operations) {
            if ((this._operationFilter && !this._operationFilter(op)) ||
                this._tracker?.shouldSkipOperation?.(op.winid, op.generation)) {
                debug('tracker', 'skipping deferred operation (user took control): ' + op.description)
                continue
            }
        ...
        // A setting may filter every deferred operation after queuing. That
        // still completes the batch; don't leave the window in RESTORING.
        if (this._tracker && !this._tracker.shouldSkipOperation?.(winid, pending.operations[0]?.generation)) {
    ...
    _executeOperations(operations) {
        ...
        while (i < operations.length) {
            ...
            if (this._tracker?.shouldSkipOperation?.(op.winid, op.generation)) {
                debug('tracker', 'skipping operation (user took control): ' + op.description)
                i++
                continue
            }
            ...
            if (op.type === 'MoveToWorkspace') {
                ...
                if (queuedOps.length > 0) {
                    this._cancelPendingOperations(winid)
                    if (this._tracker?.shouldSkipOperation?.(winid, op.generation)) continue
                    const pending = {
        ...
        try {
            ...
            switch (type) {
                ...
                case 'MoveToMonitor':
                    ...
                    break
                case 'Place':
                    // Only follow a window to its workspace when that option
                    // is enabled.  Placement itself remains valid on the
                    // current workspace; unconditional activation made the
                    // setting ineffective.
                    if (this._executor.activateWorkspace &&
                        this._executor.getDetails &&
                        this._executor._config?.activate_on_move) {
                        try {
