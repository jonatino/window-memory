 ...
 */
export function shouldTrackWindow(details, allowUninitialized = false) {
    // Must have wm_class
    if (!details.wm_class) {
        return false
    }
    ...
    // Meta.Window has no can_resize() on current GNOME. Its resizeable
    // property distinguishes fixed splash windows (including Vesktop's)
    // from the application's main window. allows_resize() alone also becomes
    // false when maximized/fullscreen, so is not an identity filter.
    if (details.resizeable === false && !details.maximized && !details.fullscreen) {
        return false
    }

    // Must be normal window type (0)
    if (details.window_type !== 0) {
        return false
    }

    // Skip transient windows (e.g. dialogs attached to a parent window)
    // These can match stale slots and drag their parent to the wrong workspace
    if (details.transient_for) {
        return false
    }

    // Must have valid geometry if frame_rect is present
    if (!allowUninitialized && details.frame_rect && !isValidGeometry(details.frame_rect)) {
        return false
    }
